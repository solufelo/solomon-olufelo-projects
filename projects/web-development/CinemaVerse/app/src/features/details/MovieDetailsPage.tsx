...
import { getMovieDetails } from 'wasp/client/operations';
import { Play, Star, Calendar, Clock, Heart, Share2, Users, Film } from 'lucide-react';
...
export default function MovieDetailsPage() {
  ...
  const { data: movieData, isLoading, error } = useQuery(getMovieDetails, { 
  ...
  const handleAddToWatchlist = async () => {
    ...
    try {
      await addToWatchlist({
        ...
        posterPath: movieData?.poster_path || undefined,
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div className="space-y-3 text-gray-300">
                        {movieData.release_date && (
                          <div className="flex justify-between">
                            ...
                            <span>{new Date(movieData.release_date).toLocaleDateString()}</span>
                          </div>
                        )}
                        {movieData.runtime && (
                          <div className="flex justify-between">
                        ...
                        {movieData.budget && movieData.budget > 0 && (
                          <div className="flex justify-between">
                            <span>Budget:</span>
                            <span>{formatCurrency(movieData.budget)}</span>
                          </div>
                        )}
                        {movieData.revenue && movieData.revenue > 0 && (
                          <div className="flex justify-between">
                            <span>Revenue:</span>
                            <span>{formatCurrency(movieData.revenue)}</span>
                          </div>
                        )}
                        {movieData.status && (
                          <div className="flex justify-between">
                            <span>Status:</span>
                            <span className="capitalize">{movieData.status.toLowerCase()}</span>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-3 text-gray-300">
                        {movieData.production_companies?.map((company: { id: number; name: string; logo_path?: string }) => (
                ...
                <div>
                  ...
                  {movieData.credits?.cast ? (
                    <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                      {movieData.credits.cast.slice(0, 12).map((person: { id: number; name: string; character: string; profile_path?: string }) => (
                ...
                <div>
                  ...
                  {movieData.reviews?.results && movieData.reviews.results.length > 0 ? (
                    <div className="space-y-4">
                      {movieData.reviews.results.slice(0, 5).map((review: { id: string; author: string; content: string; author_details: { rating?: number } }) => (
