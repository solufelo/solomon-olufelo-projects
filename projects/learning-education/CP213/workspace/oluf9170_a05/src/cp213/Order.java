...
public class Order implements Printable {
	...
	for (Map.Entry<MenuItem, Integer> entry : this.items.entrySet()) {
	    ...
	    BigDecimal itemTotal = item.getTariff().multiply(BigDecimal.valueOf(quantity));
	...
	return this.getSubTotal().add(this.getTaxes()).setScale(2, RoundingMode.HALF_UP);
    }
	...
	for (Map.Entry<MenuItem, Integer> entry : this.items.entrySet()) {
	    ...
	    int quantity = entry.getValue();
	    BigDecimal itemTotal = item.getTariff().multiply(BigDecimal.valueOf(quantity));
	    result.append(String.format(lineFormat, item.getName(), quantity, 
		    item.getTariff().doubleValue(), itemTotal.doubleValue()));
	}
	result.append("\n");
	result.append(String.format(totalFormat, "Subtotal:", this.getSubTotal().doubleValue()));
	result.append(String.format(totalFormat, "Taxes:", this.getTaxes().doubleValue()));
	result.append(String.format(totalFormat, "Total:", this.getTotal().doubleValue()));
